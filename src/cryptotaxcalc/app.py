...
class IssueReportHistoryItem(BaseModel):
    ...
    missing_expected_file_count: int | None = None
    trace_file_count: int | None = None
    raw_data_included: bool = False
    database_included: bool = False


class IssueReportHistoryResponse(BaseModel):
    ok: bool
    count: int
    reports: list[IssueReportHistoryItem]
...
def issue_report_page():
    ...
    This page does not expose admin tokens, report history, or protected support tools.
    It uses /support/report-issue/client, which creates the default sanitized bundle
    and returns the zip directly to the browser.
    """
    html = """
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>Report a problem - CryptoTaxCalc</title>
    ...
    * { box-sizing: border-box; }
    ...
    body {
      ...
      background:
        ...
        radial-gradient(circle at 90% 100%, rgba(125,211,252,.13), transparent 44%),
...
<body>
  <main>
    <a class="back-link" id="backLink" href="/workspace">&larr; Back</a>
    ...
    <section class="card">
      <div class="hero">
        ...
        <p class="sub">
          Tell us what happened and generate a safe support file you can send manually.
          The file helps diagnose calculation, import, and export issues without including
          raw CSV files or database snapshots by default.
        </p>
      ...
      <form id="issueReportForm" action="javascript:void(0)" method="post">
        <label>
          ...
          <textarea id="userMessage" name="user_message" maxlength="20000" required
            placeholder="Example: I imported a Binance CSV, clicked Calculate, and the result page looked wrong."></textarea>
        </label>
...
@app.post("/support/report-issue/client", tags=["support"])
def create_client_issue_report_bundle(req: IssueReportRequest):
    """
    Client-facing issue report creation.

    This endpoint intentionally creates only the default sanitized support bundle:
    no raw CSVs, no database snapshot, no .env, no source tree.
    It returns the zip directly so the client does not need a support/admin token.
    """
    try:
        from cryptotaxcalc.exporter import build_issue_report_bundle
        ...
        bundle_path = build_issue_report_bundle(
            ...
            app_context={
                **(req.app_context or {}),
                "surface": (req.app_context or {}).get("surface") or "client_support_page",
                "client_endpoint": "/support/report-issue/client",
            },
        )
        ...
        return FileResponse(
            ...
            filename=bundle_path.name,
            headers={
                "X-Issue-Report-Filename": bundle_path.name,
                "X-Issue-Report-Size": str(bundle_path.stat().st_size if bundle_path.exists() else 0),
            },
        )
    except Exception as e:
        logger = get_logger("support")
        logger.exception("Failed to create client issue report bundle: %s", e)
        raise HTTPException(status_code=500, detail="Could not create the support file.")
...
def _read_issue_report_index(limit: int = 20) -> list[dict[str, Any]]:
    """
    Read local issue-report index rows.
    ...
    Does not expose user messages/contact/app_context.
    """
    ...
    try:
        for line in index_path.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            if not filename.startswith("issue_report_") or not filename.endswith(".zip"):
                continue
        ...
        logger = get_logger("support")
        logger.warning("Could not read issue report index: %s", e)
        return []
...
@app.get("/support/report-issue/history", response_model=IssueReportHistoryResponse, tags=["support"])
def list_issue_report_history(
    request: Request,
    ...
    """
    List locally generated issue reports from the safe JSONL index.

    This endpoint intentionally returns only metadata generated by the local
    issue-report index. It does not read or expose issue_report.json content.
    """
    require_bundle_admin(
        request=request,
        x_admin_token=x_admin_token,
    ...
    rows = _read_issue_report_index(limit=limit)

    reports = [
        IssueReportHistoryItem(
            created_at=row.get("created_at"),
            filename=str(row.get("filename") or ""),
            path=row.get("path"),
            download_url=f"/support/report-issue/download/{row.get('filename')}",
            size_bytes=row.get("size_bytes"),
            sha256=row.get("sha256"),
            included_file_count=row.get("included_file_count"),
            missing_expected_file_count=row.get("missing_expected_file_count"),
            trace_file_count=row.get("trace_file_count"),
            raw_data_included=bool(row.get("raw_data_included", False)),
            database_included=bool(row.get("database_included", False)),
        )
        for row in rows
    ]

    return IssueReportHistoryResponse(
        ok=True,
        count=len(reports),
        reports=reports,
    )
