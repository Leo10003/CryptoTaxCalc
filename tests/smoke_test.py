...
def test_csv_upload_preview_does_not_persist_transactions():
    memo_tag = f"smoke-preview-no-persist-{uuid.uuid4().hex}"

    csv_text = f"""timestamp,type,base_asset,base_amount,quote_asset,quote_amount,fee_asset,fee_amount,exchange,memo
2022-01-01T12:00:00Z,BUY,BTC,0.10,EUR,1000,EUR,0,SmokeCSV,{memo_tag} preview buy
2024-06-01T12:00:00Z,SELL,BTC,0.04,EUR,600,EUR,0,SmokeCSV,{memo_tag} preview sell
"""
    ...
    assert endpoint.endswith("/upload/csv"), (
        f"This preview non-persistence test should use /upload/csv-compatible endpoint, got {endpoint!r}"
    )

    assert r.status_code < 500, (
        f"CSV upload preview endpoint {endpoint} must not crash on valid preview CSV. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    assert r.status_code in (200, 201, 202), (
        f"CSV upload preview endpoint {endpoint} should accept valid preview CSV. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    after_count = _count_transactions_by_memo_fragment(memo_tag)
    assert after_count == before_count, (
        f"CSV upload preview endpoint {endpoint} must not persist preview rows. "
        f"before={before_count}, after={after_count}, response={r.text[:1000]}"
    )

    ct = r.headers.get("content-type", "").lower()
    assert "application/json" in ct, (
        f"CSV upload preview endpoint {endpoint} should return JSON. "
        f"content-type={ct}, body={r.text[:1000]}"
    )
    ...
    text = json.dumps(data, default=str).lower()
    assert "btc" in text or "rows" in text or "preview" in text or "recognized_source" in text, (
        f"CSV upload preview endpoint {endpoint} should return useful preview/source information. "
        f"Response was: {data!r}"
    )


def test_csv_upload_preview_malformed_file_does_not_persist_partial_rows():
    memo_tag = f"smoke-preview-bad-no-persist-{uuid.uuid4().hex}"

    csv_text = f"""timestamp,type,base_asset,base_amount,quote_asset,quote_amount,fee_asset,fee_amount,exchange,memo
2022-01-01T12:00:00Z,BUY,BTC,0.10,EUR,1000,EUR,0,SmokeCSV,{memo_tag} valid-looking preview buy
not-a-date,SELL,BTC,0.04,EUR,600,EUR,0,SmokeCSV,{memo_tag} invalid preview sell
"""
    ...
    for endpoint in endpoints:
        files = {
            "file": (
                "smoke_preview_bad_no_persist.csv",
                csv_text.encode("utf-8"),
                "text/csv",
            )
        }
    ...
    assert endpoint.endswith("/upload/csv"), (
        f"This malformed preview non-persistence test should use /upload/csv-compatible endpoint, got {endpoint!r}"
    )

    assert r.status_code < 500, (
        f"CSV upload preview endpoint {endpoint} must not crash on malformed preview CSV. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    after_count = _count_transactions_by_memo_fragment(memo_tag)
    assert after_count == before_count, (
        f"CSV upload preview endpoint {endpoint} must not persist partial rows from malformed preview. "
        f"before={before_count}, after={after_count}, response={r.text[:1000]}"
    )

    assert r.status_code in (400, 409, 415, 422, 200, 201, 202), (
        f"Unexpected malformed preview status from {endpoint}: "
        f"{r.status_code} {r.text[:1000]}"
    )
    ...
    ct = r.headers.get("content-type", "").lower()
    assert "application/json" in ct, (
        f"CSV upload preview endpoint {endpoint} should return JSON for malformed preview feedback. "
        f"content-type={ct}, body={r.text[:1000]}"
    )
    ...
    assert _csv_response_reports_errors(data), (
        f"CSV upload preview endpoint {endpoint} should report malformed preview validation errors. "
        f"Response was: {data!r}"
    )


def test_csv_upload_preview_rejects_non_csv_filename_without_persistence():
    memo_tag = f"smoke-preview-non-csv-{uuid.uuid4().hex}"
...
"""
    ...
    assert endpoint.endswith("/upload/csv"), (
        f"This preview non-CSV test should use /upload/csv-compatible endpoint, got {endpoint!r}"
    )

    assert r.status_code < 500, (
        f"CSV upload preview endpoint {endpoint} must not crash on non-CSV filename. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    after_count = _count_transactions_by_memo_fragment(memo_tag)
    assert after_count == before_count, (
        f"CSV upload preview endpoint {endpoint} must not persist rows from non-CSV preview filename. "
        f"before={before_count}, after={after_count}, response={r.text[:1000]}"
    )

    assert r.status_code in (400, 409, 415, 422, 200, 201, 202), (
        f"Unexpected preview non-CSV filename status from {endpoint}: "
        f"{r.status_code} {r.text[:1000]}"
    )
    ...
    ct = r.headers.get("content-type", "").lower()
    assert "application/json" in ct, (
        f"CSV upload preview endpoint {endpoint} should return JSON for non-CSV filename feedback. "
        f"content-type={ct}, body={r.text[:1000]}"
    )
    ...
    assert _csv_response_reports_errors(data), (
        f"CSV upload preview endpoint {endpoint} accepted non-CSV filename without reporting errors. "
        f"Response was: {data!r}"
    )

    text = json.dumps(data, default=str).lower()
    assert ".csv" in text or "csv" in text or "file" in text, (
        f"CSV upload preview endpoint {endpoint} should explain non-CSV filename rejection. "
        f"Response was: {data!r}"
    )


def test_csv_upload_preview_accepts_uppercase_csv_extension_without_persistence():
    memo_tag = f"smoke-preview-upper-csv-{uuid.uuid4().hex}"
...
"""
    ...
    assert endpoint.endswith("/upload/csv"), (
        f"This preview uppercase .CSV test should use /upload/csv-compatible endpoint, got {endpoint!r}"
    )

    assert r.status_code < 500, (
        f"CSV upload preview endpoint {endpoint} must not crash on uppercase .CSV filename. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    assert r.status_code in (200, 201, 202), (
        f"CSV upload preview endpoint {endpoint} should accept uppercase .CSV filename. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    after_count = _count_transactions_by_memo_fragment(memo_tag)
    assert after_count == before_count, (
        f"CSV upload preview endpoint {endpoint} must not persist uppercase .CSV preview rows. "
        f"before={before_count}, after={after_count}, response={r.text[:1000]}"
    )

    ct = r.headers.get("content-type", "").lower()
    assert "application/json" in ct, (
        f"CSV upload preview endpoint {endpoint} should return JSON for uppercase .CSV preview. "
        f"content-type={ct}, body={r.text[:1000]}"
    )
...
def test_csv_import_multiple_reset_rejects_empty_filename_without_data_loss():
    existing_memo_tag = f"smoke-reset-empty-existing-{uuid.uuid4().hex}"
    bad_memo_tag = f"smoke-reset-empty-bad-{uuid.uuid4().hex}"
...
"""
    ...
    bad_csv_text = f"""timestamp,type,base_asset,base_amount,quote_asset,quote_amount,fee_asset,fee_amount,exchange,memo
2022-01-01T12:00:00Z,BUY,BTC,0.10,EUR,1000,EUR,0,SmokeCSV,{bad_memo_tag} valid content empty filename
"""

    r1, endpoint = _try_csv_import_multiple_endpoint(
        existing_csv_text,
        reset=False,
        filename="smoke_reset_empty_existing.csv",
    )
    ...
    assert r2.status_code < 500, (
        f"CSV multi-import endpoint {endpoint2} must not crash on reset empty filename. "
        f"status={r2.status_code}, body={r2.text[:1000]}"
    )
    ...
    assert existing_count_after_reset == existing_count_before_reset, (
        f"reset=true with empty filename must preserve existing rows. "
        f"before={existing_count_before_reset}, after={existing_count_after_reset}, "
        f"response={r2.text[:1000]}"
    )

    assert bad_count_after_reset == bad_count_before_reset, (
        f"reset=true with empty filename must not import replacement rows. "
        f"before={bad_count_before_reset}, after={bad_count_after_reset}, "
        f"response={r2.text[:1000]}"
    )

    assert r2.status_code in (400, 409, 415, 422, 200, 201, 202), (
        f"Unexpected reset empty-filename status from {endpoint2}: "
        f"{r2.status_code} {r2.text[:1000]}"
    )
    ...
    ct = r2.headers.get("content-type", "").lower()
    assert "application/json" in ct, (
        f"CSV multi-import endpoint {endpoint2} should return JSON for reset empty-filename feedback. "
        f"content-type={ct}, body={r2.text[:1000]}"
    )
    ...
    assert _csv_response_reports_errors(data), (
        f"CSV multi-import endpoint {endpoint2} accepted reset empty filename without reporting errors. "
        f"Response was: {data!r}"
    )

    meta = data.get("meta")
    assert isinstance(meta, dict), (
        f"reset empty-filename response should include meta. Response was: {data!r}"
    )

    assert meta.get("min_year") is None, (
        f"reset empty-filename response should not expose global min_year as imported. "
        f"Response was: {data!r}"
    )

    assert meta.get("max_year") is None, (
        f"reset empty-filename response should not expose global max_year as imported. "
        f"Response was: {data!r}"
    )

    text = json.dumps(data, default=str).lower()
    assert "filename" in text or "file" in text or ".csv" in text or "csv" in text, (
        f"CSV multi-import endpoint {endpoint2} should explain reset empty filename rejection. "
        f"Response was: {data!r}"
    )


def test_csv_upload_preview_missing_file_field_returns_clean_validation_error():
    memo_tag = f"smoke-preview-missing-file-{uuid.uuid4().hex}"
    ...
    assert endpoint.endswith("/upload/csv"), (
        f"This missing-file preview test should use /upload/csv-compatible endpoint, got {endpoint!r}"
    )

    assert r.status_code < 500, (
        f"CSV upload preview endpoint {endpoint} must not crash when multipart file field is missing. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    after_count = _count_transactions_by_memo_fragment(memo_tag)
    assert after_count == before_count, (
        f"CSV upload preview endpoint {endpoint} must not persist rows when file field is missing. "
        f"before={before_count}, after={after_count}, response={r.text[:1000]}"
    )

    assert r.status_code in (400, 409, 415, 422), (
        f"CSV upload preview endpoint {endpoint} should reject missing file field with a clean 4xx. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    ct = r.headers.get("content-type", "").lower()
    assert "application/json" in ct, (
        f"CSV upload preview endpoint {endpoint} should return JSON for missing file field. "
        f"content-type={ct}, body={r.text[:1000]}"
    )
    ...
    assert "file" in text or "field" in text or "missing" in text or "required" in text, (
        f"CSV upload preview endpoint {endpoint} should explain that the file field is missing. "
        f"Response was: {data!r}"
    )
...
def test_csv_import_multiple_reset_rejects_mixed_valid_and_empty_filename_batch_without_data_loss():
    existing_memo_tag = f"smoke-reset-mixed-empty-existing-{uuid.uuid4().hex}"
    csv_memo_tag = f"smoke-reset-mixed-empty-valid-{uuid.uuid4().hex}"
    empty_memo_tag = f"smoke-reset-mixed-empty-bad-{uuid.uuid4().hex}"

    csv_filename = "smoke_reset_mixed_empty_valid.csv"
    empty_filename = ""
...
"""
    ...
    r1, endpoint = _try_csv_import_multiple_endpoint(
        existing_csv_text,
        reset=False,
        filename="smoke_reset_mixed_empty_existing.csv",
    )
    ...
    existing_count_before_reset = _count_transactions_by_memo_fragment(existing_memo_tag)
    csv_count_before_reset = _count_transactions_by_memo_fragment(csv_memo_tag)
    empty_count_before_reset = _count_transactions_by_memo_fragment(empty_memo_tag)
    ...
    assert r2.status_code < 500, (
        f"CSV multi-import endpoint {endpoint2} must not crash on reset mixed valid/empty-filename batch. "
        f"status={r2.status_code}, body={r2.text[:1000]}"
    )

    existing_count_after_reset = _count_transactions_by_memo_fragment(existing_memo_tag)
    csv_count_after_reset = _count_transactions_by_memo_fragment(csv_memo_tag)
    empty_count_after_reset = _count_transactions_by_memo_fragment(empty_memo_tag)

    assert existing_count_after_reset == existing_count_before_reset, (
        f"reset=true mixed valid/empty-filename batch must preserve existing rows. "
        f"before={existing_count_before_reset}, after={existing_count_after_reset}, "
        f"response={r2.text[:1000]}"
    )

    assert csv_count_after_reset == csv_count_before_reset, (
        f"reset=true mixed valid/empty-filename batch must not persist replacement CSV rows. "
        f"before={csv_count_before_reset}, after={csv_count_after_reset}, "
        f"response={r2.text[:1000]}"
    )

    assert empty_count_after_reset == empty_count_before_reset, (
        f"reset=true mixed valid/empty-filename batch must not persist empty-filename replacement rows. "
        f"before={empty_count_before_reset}, after={empty_count_after_reset}, "
        f"response={r2.text[:1000]}"
    )

    assert r2.status_code in (400, 409, 415, 422, 200, 201, 202), (
        f"Unexpected reset mixed valid/empty-filename batch status from {endpoint2}: "
        f"{r2.status_code} {r2.text[:1000]}"
    )
    ...
    ct = r2.headers.get("content-type", "").lower()
    assert "application/json" in ct, (
        f"CSV multi-import endpoint {endpoint2} should return JSON for reset mixed valid/empty-filename feedback. "
        f"content-type={ct}, body={r2.text[:1000]}"
    )
    ...
    assert _csv_response_reports_errors(data), (
        f"CSV multi-import endpoint {endpoint2} accepted reset mixed valid/empty-filename batch without errors. "
        f"Response was: {data!r}"
    )

    results = data.get("results")
    assert isinstance(results, list) and len(results) >= 2, (
        f"Reset mixed valid/empty-filename batch should return one result per uploaded file. "
        f"Response was: {data!r}"
    )
    ...
    if isinstance(first_inserted, int):
        assert first_inserted == 0, (
            f"Preflight failure should report inserted=0 for valid replacement CSV file. "
            f"Response was: {data!r}"
        )

    assert _csv_response_reports_errors({"results": [results[1]]}), (
        f"Empty-filename replacement file result should report validation errors. Response was: {data!r}"
    )

    text = json.dumps(results[1], default=str).lower()
    assert "filename" in text or "file" in text or ".csv" in text or "csv" in text, (
        f"Empty-filename replacement file result should explain file/CSV validation failure. "
        f"Response was: {data!r}"
    )

    meta = data.get("meta")
    assert isinstance(meta, dict), (
        f"Reset mixed valid/empty-filename response should include meta. Response was: {data!r}"
    )

    assert meta.get("min_year") is None, (
        f"Reset mixed valid/empty-filename rejected response should not expose global min_year as imported. "
        f"Response was: {data!r}"
    )

    assert meta.get("max_year") is None, (
        f"Reset mixed valid/empty-filename rejected response should not expose global max_year as imported. "
        f"Response was: {data!r}"
    )
...
def test_csv_import_multiple_accepts_large_decimal_amounts():
    memo_tag = f"smoke-large-decimals-{uuid.uuid4().hex}"

    csv_text = f"""timestamp,type,base_asset,base_amount,quote_asset,quote_amount,fee_asset,fee_amount,exchange,memo
2022-01-01T12:00:00Z,BUY,DOGE,1000000000.12345678,EUR,999999999999.99,EUR,12345.67,SmokeCSV,{memo_tag} large buy
2024-06-01T12:00:00Z,SELL,DOGE,400000000.12345678,EUR,499999999999.99,EUR,12345.67,SmokeCSV,{memo_tag} large sell
"""
    ...
    r, endpoint = _try_csv_import_multiple_endpoint(
        csv_text,
        reset=False,
        filename="smoke_large_decimals.csv",
    )
    ...
    assert r.status_code < 500, (
        f"CSV multi-import endpoint {endpoint} must not crash on large decimal amounts. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    assert r.status_code in (200, 201, 202), (
        f"CSV multi-import endpoint {endpoint} should accept large decimal amounts. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    after_count = _count_transactions_by_memo_fragment(memo_tag)
    assert after_count - before_count == 2, (
        f"CSV multi-import endpoint {endpoint} should persist exactly 2 large-decimal rows. "
        f"before={before_count}, after={after_count}, response={r.text[:1000]}"
    )

    ct = r.headers.get("content-type", "").lower()
    assert "application/json" in ct, (
        f"CSV multi-import endpoint {endpoint} should return JSON for large decimal import. "
        f"content-type={ct}, body={r.text[:1000]}"
    )
    ...
    results = data.get("results")
    assert isinstance(results, list) and results, (
        f"Large decimal response should include results. Response was: {data!r}"
    )
    ...
    if isinstance(inserted, int):
        assert inserted == 2, (
            f"Large decimal import should report inserted=2. Response was: {data!r}"
        )

    skipped_errors = first_result.get("skipped_errors")
    if isinstance(skipped_errors, int):
        assert skipped_errors == 0, (
            f"Large decimal import should report skipped_errors=0. Response was: {data!r}"
        )

    assert not _csv_response_reports_errors(data), (
        f"Large decimal import should not report validation errors. Response was: {data!r}"
    )
...
"""
    ...
    assert endpoint.endswith("/upload/csv"), (
        f"This preview empty-filename test should use /upload/csv-compatible endpoint, got {endpoint!r}"
    )

    assert r.status_code < 500, (
        f"CSV upload preview endpoint {endpoint} must not crash on empty filename. "
        f"status={r.status_code}, body={r.text[:1000]}"
    )

    after_count = _count_transactions_by_memo_fragment(memo_tag)
    assert after_count == before_count, (
        f"CSV upload preview endpoint {endpoint} must not persist rows from empty filename. "
        f"before={before_count}, after={after_count}, response={r.text[:1000]}"
    )

    assert r.status_code in (400, 409, 415, 422, 200, 201, 202), (
        f"Unexpected preview empty-filename status from {endpoint}: "
        f"{r.status_code} {r.text[:1000]}"
    )
    ...
    ct = r.headers.get("content-type", "").lower()
    assert "application/json" in ct, (
        f"CSV upload preview endpoint {endpoint} should return JSON for empty-filename feedback. "
        f"content-type={ct}, body={r.text[:1000]}"
    )
    ...
    assert _csv_response_reports_errors(data), (
        f"CSV upload preview endpoint {endpoint} accepted empty filename without reporting errors. "
        f"Response was: {data!r}"
    )

    text = json.dumps(data, default=str).lower()
    assert "filename" in text or "file" in text or ".csv" in text or "csv" in text, (
        f"CSV upload preview endpoint {endpoint} should explain empty filename rejection. "
        f"Response was: {data!r}"
    )
